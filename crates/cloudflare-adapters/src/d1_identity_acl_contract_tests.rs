const SOURCE: &str = include_str!("d1_identity_acl.rs");
...
#[test]
fn tenant_context_projection_excludes_inactive_authority_and_is_deterministic() {
    let source = active_tenant_contexts_source();
...
#[test]
fn tenant_context_projection_cannot_cross_identity_boundary() {
    let source = active_tenant_contexts_source();
    assert!(source.contains("membership.identity_id = identity.identity_id"));
    assert!(source.contains("WHERE identity.access_subject = ?"));
    assert!(source.contains("identity.subject()"));
}
