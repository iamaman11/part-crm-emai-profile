...
pub async fn search_imap_messages(
    ...
    let next_before_uid = selected
        .last()
        .copied()
        .unwrap_or(continuation_before_uid)
        .min(continuation_before_uid.max(1));
    let next_cursor = if exhausted && next_before_uid <= 1 {
        None
    } else if next_before_uid <= 1 {
        None
    } else {
        Some(imap_query_cursor(snapshot.uid_validity, next_before_uid)?)
    };
    Ok(QueryPage::new(items, next_cursor))
}

pub async fn get_imap_message(
    binding: &MailboxBinding,
...
async fn fetch_metadata(
    ...
    let parsed_uid = parse_fetch_atom_u64(&response_text, "UID").ok_or_else(integrity_failure)?;
    if parsed_uid != uid {
        return Err(integrity_failure());
    }
    let internal_date = parse_internal_date_from_fetch(&response_text)?;
    let rfc822_size = parse_fetch_atom_u64(&response_text, "RFC822.SIZE");
...
fn parse_content_type(value: &str) -> Result<ParsedContentType, QueryPortError> {
    let mut parts = split_header_parameters(value)?;
...
fn split_header_parameters(value: &str) -> Result<impl Iterator<Item = &str>, QueryPortError> {
...
fn multipart_sections<'a>(body: &'a [u8], boundary: &[u8]) -> Result<Vec<&'a [u8]>, QueryPortError> {
    ...
    while cursor < body.len() {
        let line_start = cursor == 0 || body.get(cursor.wrapping_sub(1)) == Some(&b'\n');
        if line_start && body[cursor..].starts_with(&delimiter) {
            let after = cursor + delimiter.len();
            let closing = body.get(after..after + 2) == Some(b"--");
            let line_end = find_line_end(body, after).unwrap_or(body.len());
            starts.push((cursor, line_end, closing));
            if starts.len() > MAX_MIME_PARTS + 1 {
                return Err(integrity_failure());
            }
            if closing {
                break;
    ...
    for pair in starts.windows(2) {
        let (_, first_line_end, first_closing) = pair[0];
        if first_closing {
            break;
        }
        let (next_start, _, _) = pair[1];
        let section_start = skip_line_break(body, first_line_end);
        let section_end = trim_trailing_line_break(body, section_start, next_start);
...
fn find_line_end(body: &[u8], start: usize) -> Option<usize> {
    body.get(start..)?
        .iter()
...
fn skip_line_break(body: &[u8], line_end: usize) -> usize {
    if line_end < body.len() && body[line_end] == b'\n' {
        line_end + 1
    } else {
        line_end
    }
...
fn decode_quoted_printable(
    ...
    while index < input.len() {
        ...
        output.push((hex_value(high).ok_or_else(integrity_failure)? << 4)
...
fn decode_charset(bytes: &[u8], charset: Option<&str>) -> Result<Option<String>, QueryPortError> {
    ...
    match charset.as_str() {
        ...
        "us-ascii" | "ascii" => {
            ...
            Ok(Some(String::from_utf8(bytes.to_vec()).map_err(|_| integrity_failure())?))
        }
...
fn parse_imap_internal_date(value: &str) -> Option<UnixMillis> {
    ...
    let zone_hours = core::str::from_utf8(&zone_bytes[1..3]).ok()?.parse::<i64>().ok()?;
    let zone_minutes = core::str::from_utf8(&zone_bytes[3..5]).ok()?.parse::<i64>().ok()?;
    if zone_hours > 23 || zone_minutes > 59 {
        return None;
    }
    let zone_seconds = zone_hours.checked_mul(3600)?.checked_add(zone_minutes * 60)?;
...
fn extract_first_literal(response: &[u8]) -> Result<Option<&[u8]>, QueryPortError> {
    ...
    let data_end = data_start.checked_add(length).ok_or_else(integrity_failure)?;
    let data = response.get(data_start..data_end).ok_or_else(integrity_failure)?;
    Ok(Some(data))
}
...
mod tests {
    use super::{
        decode_quoted_printable, extract_first_literal, multipart_sections, parse_imap_internal_date,
        parse_search_uids,
    };
    ...
    fn imap_internal_date_respects_numeric_zone() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(utc.value(), 1_754_759_600_000);
        Ok(())
    }
    ...
    #[test]
    fn quoted_printable_decoder_handles_soft_breaks() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(
