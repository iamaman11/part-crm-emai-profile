...
impl ProfileGrantApplicationPort for D1ProfileApplicationRepository {
    async fn decide_profile_grant_replay(
        ...
        self.idempotency
            ...
            .await
            .map(map_replay_decision)
            .map_err(map_profile_grant_dependency_error)
    }
    ...
    async fn grant_profile(
        ...
        self.write_profile_grant(actor, write, false).await
    }
    ...
    async fn revoke_profile_grant(
        ...
        self.write_profile_grant(actor, write, true).await
    }
}

impl D1ProfileApplicationRepository {
    async fn write_profile_grant(
        &self,
        actor: &ActorContext,
        write: &ProfileGrantWrite,
        revoke: bool,
        ...
        let mutation = ProfileGrantMutation {
            ...
            profile_id: write.profile_id(),
            expected_profile_version: write.expected_profile_version(),
            role: map_profile_grant_role(write.role()),
            reason: write.reason(),
            envelope: mutation_envelope(evidence, write.event_payload_json()),
        };
        let result = if revoke {
            self.governed.revoke_profile_grant(actor, mutation).await
        } else {
...
const fn map_profile_grant_role(role: ProfileGrantRole) -> ProfileGrantValue {
    match role {
        ProfileGrantRole::Viewer => ProfileGrantValue::Viewer,
        ProfileGrantRole::Operator => ProfileGrantValue::Operator,
    }
}
...
fn map_profile_grant_write_error(error: Error) -> ProfileGrantPortError {
    ProfileGrantPortError::new(classify_profile_grant_write_failure(&error.to_string()))
}

fn map_profile_grant_dependency_error(_error: Error) -> ProfileGrantPortError {
    ProfileGrantPortError::new(ProfileGrantPortErrorClass::DependencyUnavailable)
}

fn classify_profile_grant_write_failure(message: &str) -> ProfileGrantPortErrorClass {
    if message.contains("owner_required")
        || message.contains("profile_missing")
        || message.contains("target_missing")
        || message.contains("target_not_active_member")
    {
        return ProfileGrantPortErrorClass::NotFound;
    }
    if message.contains("state_mismatch")
        || message.contains("version_mismatch")
        || message.contains("tenant_version_mismatch")
    {
        return ProfileGrantPortErrorClass::VersionConflict;
    }
    if message.contains("time_regression")
        || message.contains("invalid_transition")
        || message.contains("grant_missing")
    {
        return ProfileGrantPortErrorClass::InvalidState;
    }
    if message.contains("UNIQUE constraint failed") {
        return ProfileGrantPortErrorClass::Conflict;
    }
    if message.contains("CHECK constraint failed")
        || message.contains("FOREIGN KEY constraint failed")
        || message.contains("not_governed")
    {
        return ProfileGrantPortErrorClass::IntegrityFailure;
    }
    if message.contains("aggregate version overflow")
        || message.contains("value exceeds SQLite INTEGER")
        || message.contains("idempotency expiry overflow")
    {
        return ProfileGrantPortErrorClass::InternalFailure;
    }
...
mod tests {
    use super::{
        classify_assignment_write_failure, classify_profile_grant_write_failure,
        classify_write_failure,
    };
    use application_ports::profiles::{
        ProfileAssignmentPortErrorClass, ProfileGrantPortErrorClass, ProfilePortErrorClass,
    };
    ...
    #[test]
    fn profile_grant_write_failures_keep_legacy_public_classes() {
        assert_eq!(
            classify_profile_grant_write_failure("profile_grant_profile_missing"),
            ProfileGrantPortErrorClass::NotFound
        );
        assert_eq!(
            classify_profile_grant_write_failure("profile_grant_target_not_active_member"),
            ProfileGrantPortErrorClass::NotFound
        );
        assert_eq!(
            classify_profile_grant_write_failure("profile_grant_profile_version_mismatch"),
            ProfileGrantPortErrorClass::VersionConflict
        );
        assert_eq!(
            classify_profile_grant_write_failure("profile_grant_missing"),
            ProfileGrantPortErrorClass::InvalidState
        );
        assert_eq!(
            classify_profile_grant_write_failure(
                "UNIQUE constraint failed: profile_grant_commands.tenant_id"
            ),
            ProfileGrantPortErrorClass::Conflict
        );
        assert_eq!(
            classify_profile_grant_write_failure("profile_grant_not_governed"),
            ProfileGrantPortErrorClass::IntegrityFailure
        );
        assert_eq!(
            classify_profile_grant_write_failure("aggregate version overflow"),
            ProfileGrantPortErrorClass::InternalFailure
        );
        assert_eq!(
            classify_profile_grant_write_failure("network request failed"),
            ProfileGrantPortErrorClass::DependencyUnavailable
        );
    }
}
