...
use application_ports::gmail_oauth_onboarding::{
    ...
    GmailOAuthCeremonyId, GmailOAuthInputError, GmailOAuthProvisioningError,
...
impl FakeProvisioningPort {
    fn new(target: GmailOAuthCallbackTarget) -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self {
            receipt: GmailOAuthStartReceipt::new(
                oauth_input(
                    GmailOAuthCeremonyId::parse("ceremony_C2_behavior"),
                    "invalid ceremony id",
                )?,
                oauth_input(
            ...
            target,
            completion_handle: SecretHandle::parse("secret_C2_new")?,
...
impl GmailOAuthProvisioningPort for FakeProvisioningPort {
    fn start(
        &self,
        _actor: &ActorContext,
        _onboarding_id: &MailboxOnboardingId,
        _expected_version: MailboxOnboardingVersion,
    ) -> impl Future<Output = Result<GmailOAuthStartReceipt, GmailOAuthProvisioningError>> {
    ...
    fn inspect(
        &self,
        _state: &GmailOAuthState,
    ) -> impl Future<Output = Result<GmailOAuthCallbackTarget, GmailOAuthProvisioningError>> {
    ...
    fn complete(
        ...
        _actor: &ActorContext,
        _state: &GmailOAuthState,
        _authorization_code: GmailOAuthAuthorizationCode,
    ) -> impl Future<Output = Result<SecretHandle, GmailOAuthProvisioningError>> {
    ...
    fn deny(
        ...
        _actor: &ActorContext,
        _state: &GmailOAuthState,
    ) -> impl Future<Output = Result<(), GmailOAuthProvisioningError>> {
...
fn pending_and_reauth_start_are_owner_only_and_versioned() -> TestResult {
    let actor = actor("tenant_C2_owner", "actor_C2_owner")?;

    let pending_id = MailboxOnboardingId::parse("onboarding_C2_pending")?;
    ...
    let pending = block_on(start_gmail_oauth_onboarding(
        &actor,
    ...
    let denied_id = MailboxOnboardingId::parse("onboarding_C2_member")?;
    ...
    let stale_id = MailboxOnboardingId::parse("onboarding_C2_stale")?;
    ...
    assert_eq!(
        ...
        Err(GmailOAuthOnboardingError::VersionConflict)
    );
...
fn invalid_provider_and_terminal_or_active_states_never_touch_resolver() -> TestResult {
    let actor = actor("tenant_C2_invalid", "actor_C2_owner")?;
    let onboarding_id = MailboxOnboardingId::parse("onboarding_C2_invalid")?;
    ...
    assert_eq!(
        ...
        Err(GmailOAuthOnboardingError::InvalidState)
    );
...
fn callback_is_bound_to_exact_tenant_and_starter_actor() -> TestResult {
    let actor = actor("tenant_C2_callback", "actor_C2_owner")?;
    let onboarding_id = MailboxOnboardingId::parse("onboarding_C2_callback")?;
    let state = oauth_input(
        GmailOAuthState::parse("state_C2_0123456789abcdef"),
        "invalid state",
    )?;

    let foreign_target = GmailOAuthCallbackTarget::new(
        TenantId::parse("tenant_C2_foreign")?,
        onboarding_id.clone(),
        MailboxOnboardingVersion::INITIAL,
    ...
    let other_actor_target = GmailOAuthCallbackTarget::new(
        actor.tenant_scope().tenant_id().clone(),
        onboarding_id.clone(),
        MailboxOnboardingVersion::INITIAL,
        ActorId::parse("actor_C2_other")?,
        UnixMillis::new(10_000),
    );
...
fn completion_activates_exact_version_and_discards_handle_after_failed_commit() -> TestResult {
    let actor = actor("tenant_C2_complete", "actor_C2_owner")?;
    let onboarding_id = MailboxOnboardingId::parse("onboarding_C2_complete")?;
    ...
    let success = block_on(complete_gmail_oauth_callback(
        &actor,
        MembershipRole::TenantOwner,
        &success_port,
    ...
    assert_eq!(
        block_on(complete_gmail_oauth_callback(
            &actor,
            MembershipRole::TenantOwner,
            &failed_port,
            &failed_provisioning,
...
fn actor(tenant_id: &str, actor_id: &str) -> Result<ActorContext, Box<dyn std::error::Error>> {
    Ok(ActorContext::new(
        ...
        CorrelationId::parse("corr_C2_behavior")?,
    ))
}
...
fn onboarding(
    ...
    let credential_handle = if status == MailboxOnboardingStatus::Pending {
        ...
        Some(SecretHandle::parse("secret_C2_existing")?)
    };
...
fn callback_target(
    ...
    onboarding_id: MailboxOnboardingId,
    version: MailboxOnboardingVersion,
) -> GmailOAuthCallbackTarget {
    GmailOAuthCallbackTarget::new(
        actor.tenant_scope().tenant_id().clone(),
        onboarding_id,
        version,
        actor.actor_id().clone(),
...
fn authorization_code() -> Result<GmailOAuthAuthorizationCode, Box<dyn std::error::Error>> {
    oauth_input(
        GmailOAuthAuthorizationCode::parse("code_C2_transient"),
...
fn oauth_input<T>(
    result: Result<T, GmailOAuthInputError>,
    message: &'static str,
