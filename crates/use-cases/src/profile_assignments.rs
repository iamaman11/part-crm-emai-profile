...
pub async fn execute_assign_profile<P: ProfileAssignmentApplicationPort>(
    actor: &ActorContext,
    role: MembershipRole,
    port: &P,
...
mod tests {
    ...
    fn command(
        ...
        Ok(ExecuteAssignProfileCommand::new(
            ...
            ClientId::parse("client_01JASSIGNAPP")?,
            version,
    ...
    #[test]
    fn member_is_rejected_before_replay_or_write() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            block_on(execute_assign_profile(
                ...
                &port,
                command(AggregateVersion::INITIAL)?,
            )),
            Err(ProfileAssignmentOperationError::NotFound)
        );
    ...
    #[test]
    fn overflow_is_rejected_before_replay_or_write() -> Result<(), Box<dyn std::error::Error>> {
        let port = FakePort::new(vec![ProfileReplayDecision::Miss]);
        assert_eq!(
            block_on(execute_assign_profile(
                &actor()?,
                ...
                &port,
                command(AggregateVersion::new(u64::MAX)?)?,
            )),
            Err(ProfileAssignmentOperationError::InternalFailure)
        );
        assert_eq!(port.replay_calls.get(), 0);
        assert_eq!(port.write_calls.get(), 0);
        Ok(())
    ...
    #[test]
    fn fresh_assignment_writes_once() -> Result<(), Box<dyn std::error::Error>> {
        let port = FakePort::new(vec![ProfileReplayDecision::Miss]);
    ...
    #[test]
    fn exact_prewrite_replay_skips_write() -> Result<(), Box<dyn std::error::Error>> {
        let port = FakePort::new(vec![ProfileReplayDecision::Replay(
            ProfileReplayReceipt::new("assigned", Some("assignment_existing".to_owned())),
        )]);
        let outcome = block_on(execute_assign_profile(
            &actor()?,
            MembershipRole::TenantOwner,
    ...
    fn non_conflict_write_failure_never_rechecks_replay() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        port.write_error
            .set(Some(ProfileAssignmentPortErrorClass::VersionConflict));
        assert_eq!(
            block_on(execute_assign_profile(
                &actor()?,
                MembershipRole::TenantOwner,
