...
const CLIENT_REGISTRY_SCHEMA_NAMES: [&str; 13] = [
    "ClientContactKind", "ClientContactStatus", "ClientAssignmentStatus", "ClientListProjection",
    "ClientUpdateRequest", "ClientArchiveRequest", "ClientContactUpsertRequest",
    "ClientContactArchiveRequest", "ClientMergeRequest", "ClientContactProjection",
    "ClientAssignmentProjection", "ClientActivityProjection", "ClientHistoryProjection",
];
...
pub struct ClientListProjection { pub clients: Vec<ClientProjection> }
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ClientArchiveRequest { pub expected_client_version: u64 }
...
fn extend_openapi_in_place(document: &mut Value) -> Result<(), ClientRegistryOpenApiError> {
    let paths = document.get_mut("paths").and_then(Value::as_object_mut).ok_or(ClientRegistryOpenApiError::MissingPathsObject)?;
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients", "get", json!({
    ...
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}", "patch", json!({
    ...
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}/archive", "post", json!({
    ...
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}/contacts/{contactPointId}", "put", json!({
    ...
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}/contacts/{contactPointId}", "delete", json!({
        "operationId": "archiveClientContact",
        "parameters": [tenant_path_parameter(), opaque_path_parameter("clientId"), opaque_path_parameter("contactPointId")],
        "requestBody": json_request("ClientContactArchiveRequest"), "responses": mutation_responses()
    }))?;
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}/merge", "post", json!({
    ...
    insert_operation(paths, "/api/v1/tenants/{tenantId}/clients/{clientId}/history", "get", json!({
    ...
    let schemas = document.get_mut("components").and_then(|value| value.get_mut("schemas")).and_then(Value::as_object_mut).ok_or(ClientRegistryOpenApiError::MissingSchemasObject)?;
    if CLIENT_REGISTRY_SCHEMA_NAMES.iter().any(|name| schemas.contains_key(*name)) {
        return Err(ClientRegistryOpenApiError::DuplicateSchema);
    }
    schemas.insert("ClientContactKind".to_owned(), string_enum(&CLIENT_CONTACT_KINDS));
    schemas.insert("ClientContactStatus".to_owned(), string_enum(&CLIENT_CONTACT_STATUSES));
    schemas.insert("ClientAssignmentStatus".to_owned(), string_enum(&CLIENT_ASSIGNMENT_STATUSES));
    ...
    schemas.insert("ClientUpdateRequest".to_owned(), object_schema(&["displayName", "expectedClientVersion"], json!({
        "displayName": {"type": "string", "minLength": 1, "maxLength": 200},
        "expectedClientVersion": positive_version_schema()
    })));
    schemas.insert("ClientArchiveRequest".to_owned(), object_schema(&["expectedClientVersion"], json!({
        "expectedClientVersion": positive_version_schema()
    })));
...
fn insert_operation(paths: &mut Map<String, Value>, path: &str, method: &str, operation: Value) -> Result<(), ClientRegistryOpenApiError> {
    let path_item = paths.entry(path.to_owned()).or_insert_with(|| Value::Object(Map::new()));
    let path_item = path_item.as_object_mut().ok_or(ClientRegistryOpenApiError::InvalidPathItem)?;
    if path_item.contains_key(method) { return Err(ClientRegistryOpenApiError::DuplicateOperation); }
...
fn positive_version_schema() -> Value { json!({"type": "integer", "format": "uint64", "minimum": 1}) }
fn non_negative_time_schema() -> Value { json!({"type": "integer", "format": "uint64", "minimum": 0}) }
fn nullable_non_negative_time_schema() -> Value {
    json!({"oneOf": [{"type": "integer", "format": "uint64", "minimum": 0}, {"type": "null"}]})
}
fn string_enum(values: &[&str]) -> Value { json!({"type": "string", "enum": values}) }
fn schema_ref(name: &str) -> Value { json!({"$ref": format!("#/components/schemas/{name}")}) }
fn tenant_path_parameter() -> Value { opaque_path_parameter("tenantId") }
...
mod tests {
    ...
    fn client_registry_models_keep_camel_case_wire_names() -> Result<(), Box<dyn std::error::Error>> {
        let update = serde_json::to_value(ClientUpdateRequest { display_name: "Renamed".to_owned(), expected_client_version: 2 })?;
        ...
        let history = serde_json::to_value(ClientHistoryProjection {
            contacts: vec![ClientContactProjection { contact_point_id: "contact_01JCONTRACT".to_owned(), kind: "EMAIL".to_owned(), status: "ACTIVE".to_owned() }],
            assignments: vec![ClientAssignmentProjection {
                assignment_id: "assignment_01JCONTRACT".to_owned(), profile_id: "profile_01JCONTRACT".to_owned(), status: "ACTIVE".to_owned(),
                assigned_at_ms: 10, closed_at_ms: None, reason: "primary".to_owned(),
            }],
            activity: Vec::new(),
        })?;
    ...
    #[test]
    fn client_registry_openapi_extension_is_additive_and_complete() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for name in [
            "ClientListProjection", "ClientUpdateRequest", "ClientArchiveRequest", "ClientContactUpsertRequest",
            "ClientContactArchiveRequest", "ClientMergeRequest", "ClientContactProjection", "ClientAssignmentProjection",
            "ClientActivityProjection", "ClientHistoryProjection",
        ] {
            assert!(schemas.get(name).is_some(), "missing schema {name}");
        }
        for name in ["ClientUpdateRequest", "ClientArchiveRequest", "ClientContactUpsertRequest", "ClientContactArchiveRequest", "ClientMergeRequest"] {
            assert!(schemas[name]["properties"].get("requestDigest").is_none(), "legacy requestDigest survived in {name}");
        }
        Ok(())
    }
    ...
    fn malformed_and_duplicate_openapi_extensions_fail_atomically() {
        ...
        assert_eq!(extend_openapi(&mut missing_paths), Err(super::ClientRegistryOpenApiError::MissingPathsObject));
        ...
        assert_eq!(extend_openapi(&mut duplicate), Err(super::ClientRegistryOpenApiError::DuplicateOperation));
        assert_eq!(duplicate, original);
    }

    #[test]
    fn request_models_reject_unknown_and_legacy_digest_fields() {
        assert!(serde_json::from_str::<ClientArchiveRequest>(r#"{"expectedClientVersion":1}"#).is_ok());
        assert!(serde_json::from_str::<ClientArchiveRequest>(r#"{"expectedClientVersion":1,"requestDigest":"legacy"}"#).is_err());
        assert!(serde_json::from_str::<ClientContactArchiveRequest>(r#"{"kind":"EMAIL","expectedClientVersion":1,"extra":true}"#).is_err());
    }

    #[test]
    fn other_registry_request_models_are_constructible() {
        let _list = ClientListProjection { clients: vec![ClientProjection {
            client_id: "client_01JCONTRACT".to_owned(), kind: "PERSON".to_owned(), display_name: "Client".to_owned(), status: "ACTIVE".to_owned(), version: 1,
        }]};
        let _contact_upsert = ClientContactUpsertRequest { kind: "EMAIL".to_owned(), value: "person@example.com".to_owned(), expected_client_version: 1 };
        let _merge = ClientMergeRequest {
            target_client_id: "client_02JCONTRACT".to_owned(), expected_source_version: 1, expected_target_version: 1, reason: "duplicate".to_owned(),
        };
    }
}
