...
impl NetworkIdentityPolicy {
    pub fn new(
        ...
        validate_optional_location(country.as_deref())?;
        validate_optional_location(region.as_deref())?;
        if timezone
            .as_deref()
...
fn validate_optional_location(value: Option<&str>) -> Result<(), BrowserExecutionError> {
...
mod tests {
    ...
    #[test]
    fn materialization_binding_rejects_generation_or_identity_substitution()
    -> Result<(), Box<dyn std::error::Error>> {
