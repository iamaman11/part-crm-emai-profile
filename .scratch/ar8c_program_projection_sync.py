...
def patch_generator() -> None:
    ...
    replace_once(
        generator,
        "docs/status.json must project AR-8A accepted / AR-8B current / AR-8C..F mandatory",
        "docs/status.json must project AR-8A/B accepted / AR-8C current / AR-8D..F mandatory",
    )

    helper = '''def build_ar8c_operational_lifecycle_projection(payload: dict[str, Any]) -> dict[str, object]:
...
'''
    replace_once(
        ...
        helper + "def build_credential_projection(payload: dict[str, Any]) -> dict[str, object]:\n",
    )
    replace_once(
        generator,
    ...
    replace_once(
        generator,
        "inventory self-test failed to detect AR-8B sequencing drift",
        "inventory self-test failed to detect AR-8C sequencing drift",
    )


def patch_documentation_checker() -> None:
    checker = "scripts/check-documentation-authority.py"
    replace_once(
        checker,
        'AR8B_IMPLEMENTATION_ISSUE = 309\nAR8_ACCEPTED_SUBSLICES = ["AR-8A"]\nAR8_CURRENT_SUBSLICE = "AR-8B"\nAR8_MANDATORY_REMAINING = ["AR-8B", "AR-8C", "AR-8D", "AR-8E", "AR-8F"]',
        'AR8B_IMPLEMENTATION_ISSUE = 309\nAR8C_IMPLEMENTATION_ISSUE = 314\nAR8_ACCEPTED_SUBSLICES = ["AR-8A", "AR-8B"]\nAR8_CURRENT_SUBSLICE = "AR-8C"\nAR8_MANDATORY_REMAINING = ["AR-8C", "AR-8D", "AR-8E", "AR-8F"]\nAR8B_ACCEPTANCE = {\n    "issue": 309,\n    "implementation_pr": 312,\n    "exact_green_head": "743276b578bb15042e8a42beff0e14f7698e61b0",\n    "implementation_merge": "4e4d1c25226384858ca8905377ee155bedabc6d4",\n    "applicable_permanent_workflows": "14/14",\n}',
    ...
    helper = '''def expected_ar8c_operational_lifecycle_projection(payload: dict[str, Any]) -> dict[str, object]:
    lifecycle = payload.get("ar8c_operational_lifecycle")
    if not isinstance(lifecycle, dict):
...
'''
    replace_once(
        ...
        helper + "def expected_credential_projection(root: Path, payload: dict[str, Any]) -> dict[str, object]:\n",
    )
    replace_once(
        checker,
    ...
    replace_once(
        checker,
        "Architecture Re-baseline v3 active AR-8 / current AR-8B documentation authority negative fixtures passed.",
        "Architecture Re-baseline v3 active AR-8 / current AR-8C documentation authority negative fixtures passed.",
    )
    replace_once(
        checker,
        "Architecture Re-baseline v3 active AR-8 / current AR-8B documentation/program authority is consistent.",
        "Architecture Re-baseline v3 active AR-8 / current AR-8C documentation/program authority is consistent.",
    )


def patch_json_projections() -> None:
    acceptance = {
        "issue": 309,
        "implementation_pr": 312,
        "exact_green_head": "743276b578bb15042e8a42beff0e14f7698e61b0",
    ...
    progress = {
        "umbrella_issue": 308,
        "accepted_subslices": ["AR-8A", "AR-8B"],
        "current_subslice": "AR-8C",
        "current_implementation_issue": 314,
        "mandatory_remaining": ["AR-8C", "AR-8D", "AR-8E", "AR-8F"],
    ...
    program["ar8_progress"] = progress
    program["ar8b_acceptance"] = acceptance
    status["next_repository_step"] = {
        "number": None,
        "name": "Architecture Re-baseline v3 AR-8C",
        "status": "current_ar8_subslice_after_accepted_ar8b",
        "tracking_issue": 266,
        "umbrella_issue": 308,
    ...
    transition_path = Path("architecture/architecture-rebaseline-v3-transition.json")
    transition = json.loads(transition_path.read_text(encoding="utf-8"))
    transition["schema_version"] = 11
    transition["status"] = "ACTIVE_DURING_AR8_AFTER_ACCEPTED_AR8B"
    transition_progress = dict(progress)
    transition_progress["credential_authority_source"] = (
        "architecture/credential-authority-ar8b.json"
    )
    transition_progress["canonical_projection"] = (
        "architecture/inventory.json::credential_authority"
    )
    transition["ar8_progress"] = transition_progress
    transition["ar8b_acceptance"] = acceptance
    policy = transition["architecture_inventory_policy"]
    policy.pop("ar8b_current_projection", None)
    policy["ar8b_accepted_projection"] = (
        "ACCEPTED_METADATA_ONLY_CREDENTIAL_AUTHORITY_IN_CANONICAL_INVENTORY"
    )
    policy["ar8c_current_projection"] = (
        "CANDIDATE_OPERATIONAL_CREDENTIAL_LIFECYCLE_IN_CANONICAL_INVENTORY"
    )
    app = transition["application_architecture"]
    app["program_handoff_status"] = "AR8_ACTIVE_AFTER_AR8B_ACCEPTED"
    app["program_next_required_subslice"] = "AR-8C"
    transition_path.write_text(
        json.dumps(transition, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )


def main() -> None:
    patch_generator()
    patch_documentation_checker()
    patch_json_projections()
