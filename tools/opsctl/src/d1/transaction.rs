...
pub fn build_transaction_projection(
    ...
    let observation_input: ProviderObservationInput = typed(observation_value, "provider observation")?;
    let transaction_input: TransactionIdentityInput = typed(transaction_value, "transaction identity input")?;
    ...
    let canonical_plan = canonical_json(&serde_json::to_value(&transaction_plan).map_err(|error| {
        D1Error::new(format!("cannot serialize migration transaction plan: {error}"))
    })?)
    .map_err(D1Error::new)?;
...
pub fn serialize_transaction_projection(projection: &TransactionProjection) -> Result<String, D1Error> {
    ...
    T: for<'de> Deserialize<'de>,
{
    serde_json::from_value(value.clone()).map_err(|error| {
        D1Error::new(format!("{label} does not match the typed contract: {error}"))
    })
}
...
fn seal_observation(input: ProviderObservationInput) -> Result<ProviderObservationBundle, D1Error> {
    ...
    if input.remote_migrations.iter().collect::<BTreeSet<_>>().len() != input.remote_migrations.len() {
        return Err(D1Error::new("remote_migrations must not contain duplicates"));
    }
    if input.wrangler_pending_migrations.iter().collect::<BTreeSet<_>>().len()
...
fn repository_identity(
    ...
    validate_sha256(
        required_str(repository, "repository_identity_sha256")?,
    ...
    let authority = components
        .iter()
        .find(|candidate| {
            candidate.get("component_id").and_then(Value::as_str) == Some(component)
        })
...
fn validate_transaction_input(
    ...
    validate_sha256(&input.predecessor_ledger_sha256, "predecessor_ledger_sha256")?;
...
mod tests {
    ...
    fn identical_inputs_produce_identical_transaction_identity() {
        let left = build_transaction_projection(
            &prepare(),
            &observation(),
            &repository(),
            &transaction_input(),
        ...
        .unwrap();
        let right = build_transaction_projection(
            &prepare(),
            &observation(),
            &repository(),
            &transaction_input(),
        ...
        assert_eq!(left.transaction_plan.migration_lineage_digest, "11".repeat(32));
    ...
    fn provider_observation_drift_changes_transaction_identity() {
        let baseline = build_transaction_projection(
            &prepare(),
            &observation(),
            &repository(),
            &transaction_input(),
    ...
    fn repository_lineage_drift_is_rejected() {
        ...
        assert!(build_transaction_projection(
            &prepare(),
            &observation(),
            &changed,
            &transaction_input(),
