...
const PREDECESSOR_REVISION: &str = "0034_device_application_authority.sql";
const CONTRACT_REVISION: &str = "0035_pas2_payload_fingerprint_contract.sql";
...
fn validate_release_contract(
    ...
    if authority.current_repository_revision != CONTRACT_REVISION {
        return Err(D1Error::new(
            "Catalog repository authority does not end at the governed 0035 CONTRACT",
        ));
    }
    ...
    {
        return Err(D1Error::new(
            "contract-transition release schema window must be exact device-application-0034..PAS2-0035 and match typed repository identity",
        ));
    }
    let contract = authority
        .post_epoch
        .last()
        .ok_or_else(|| D1Error::new("Catalog 0035 CONTRACT metadata is missing"))?;
    ...
    {
        return Err(D1Error::new(
            "Catalog 0035 CONTRACT metadata drifted from the fail-forward separate-release invariant",
        ));
    }
    Ok(())
}
...
fn validate_exact_predecessor(
    ...
    let contract_index = authority
        ...
        .ok_or_else(|| D1Error::new("Catalog 0035 CONTRACT is absent from typed history"))?;
    if contract_index == 0 || authority.ordered_history[contract_index - 1] != PREDECESSOR_REVISION
    {
        return Err(D1Error::new(
            "Catalog 0035 CONTRACT does not have the exact governed device-application-0034 predecessor",
        ));
    }
    if remote_names != &authority.ordered_history[..contract_index] {
        return Err(D1Error::new(
            "contract-transition requires the remote ledger to equal the exact canonical prefix through device-application 0034",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn post_contract_verification_requires_exactly_one_contract_step() {
        ...
        extra.push("0036_unexpected.sql".to_owned());
