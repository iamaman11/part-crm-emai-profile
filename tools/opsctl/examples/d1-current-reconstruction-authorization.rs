...
use opsctl::d1::authorization::{
    bind_current_reconstruction_authorization, serialize_authorization_binding,
};
...
fn parse_args() -> Result<Args, Box<dyn Error>> {
    ...
    while let Some(argument) = iterator.next() {
        let flag = argument
            .to_str()
            .ok_or("reconstruction-authorization flags must be valid UTF-8")?;
        match flag {
...
fn run() -> Result<(), Box<dyn Error>> {
    let args = parse_args()?;
    let reconstruction = read_strict(
        required(args.reconstruction_json, "--reconstruction-json")?,
