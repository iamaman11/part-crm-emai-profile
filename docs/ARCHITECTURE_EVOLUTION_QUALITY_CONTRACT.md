...
Functional Closure remains focused on proving the accepted AR-11 behavior. It should consume the PF-1/PF-2 primitives and must not create temporary parallel operational authorities. FC-7 also confirms the prospective contract is correctly reflected in the current program/development projections before AR-12 implementation begins.
...
- add the first repository-level dependency/effect fitness rules where rehearsal wiring exposes real boundary risks;
