...
| Capability add/enable/disable/remove and runtime admission | `crates/capability-policy` plus the owning product decision and runtime ingress/composition adapter | mandatory product-scope/profile-impact disposition, typed profile/surface catalog, dependency/digest/admission tests, ingress mapping tests and exact release/effective-set evidence | source alone never enables effects; an accepted scope change cannot proceed downstream while the selected effective set disagrees; used profile identities remain immutable; unknown surface/profile/environment/authorization fails closed |
