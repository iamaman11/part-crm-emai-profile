...
| Complete standalone UI | Composed / Synthetic | Phase 2H implementation PR #164 was accepted from exact source head `9add9b94d0de255b93e5a7c24584fcf6756462a7` and guarded squash merge `a32768feddb3da69b872e701bc529aad3521e1b0`, with exactly 12/12 permanent workflows success, `behind_by=0`, reviews=0 and unresolved review threads=0. | Phase 2I owns integrated E2E/security/recovery/operations hardening; Phase 2J closes real rollout evidence. |
...
| A5 | Feature-sliced SPA route composition | **Accepted through Phase 2H** — Phase 2C established feature-owned public route APIs; Phase 2H expands all required route families while root composition continues to reject sibling-feature internals. | Preserve continuously through Phase 2I. |
...
| A8 | CQRS/read-model boundary | **Accepted through Phase 2H** — independent `use-cases-query`, capability-owned read projections and authorization-before-projection/provider sequencing remain authoritative; Phase 2H exposes accepted projections through thin generated transport/UI without moving query policy outward. | Preserve through Phase 2I. |
...
| 6.2 | Durable-before-notify | **Accepted through Phase 2H.** Phase 1B durable delivery remains canonical; Phase 2G live fanout occurs only after durable `Delivered`; Phase 2H UI continues to treat realtime as invalidation/refetch rather than success authority. | Preserve through Phase 2I. |
| 6.3 | At-least-once consumer idempotency | **Accepted through Phase 2H.** Notification replay, mailbox/device consumers and realtime duplicate delivery remain duplicate-neutral at canonical state/UI logical-state boundaries. | Preserve for every later consumer/surface. |
| 6.4 | Authorization-before-projection | **Accepted through Phase 2H.** Live membership/grants precede projection/provider access and realtime subscription/catch-up; Phase 2H operator lists and Client Mail transport call accepted authorization/query application paths rather than recreating ACL in UI. | Preserve continuously through Phase 2I. |
