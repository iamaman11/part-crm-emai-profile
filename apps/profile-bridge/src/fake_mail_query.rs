...
const BRIDGE_MESSAGE_REFERENCE: &str = "synthetic-bridge-message-1";
const BRIDGE_BODY: &str = "Synthetic confidential Bridge message body";

#[derive(Clone, Copy, Default)]
pub struct DeterministicFakeBridgeMailQueryAdapter;

impl ClientMailProviderQueryPort for DeterministicFakeBridgeMailQueryAdapter {
    async fn search_messages(
        ...
        let reference = MailboxMessageReference::new(binding_id.clone(), BRIDGE_MESSAGE_REFERENCE)
            .map_err(|_| integrity_error())?;
        Ok(QueryPage::new(
            vec![MailMessageSummary::new(
                reference,
                Some("Synthetic Bridge subject".to_owned()),
                Some("synthetic-bridge@example.invalid".to_owned()),
                UnixMillis::new(200),
            )],
            None,
        ))
    ...
    async fn get_message(
        ...
        if reference.provider_reference() != BRIDGE_MESSAGE_REFERENCE {
            return Ok(None);
        }
        ...
        MailMessageBody::new(
            summary,
            Some(BRIDGE_BODY.to_owned()),
            Some(format!("<p>{BRIDGE_BODY}</p>")),
        )
        .map(Some)
        .map_err(|_| integrity_error())
    }
...
#[cfg(test)]
mod tests {
    use super::{BRIDGE_BODY, DeterministicFakeBridgeMailQueryAdapter};
    ...
    #[test]
    fn fake_bridge_adapter_returns_full_synthetic_body_without_runtime_execution()
    -> Result<(), Box<dyn std::error::Error>> {
        let adapter = DeterministicFakeBridgeMailQueryAdapter;
        let scope = TenantScope::new(TenantId::parse("tenant_01JFAKEBRIDGE")?);
        let binding = MailboxBindingId::parse("binding_01JFAKEBRIDGE")?;
        ...
        let body = block_on(adapter.get_message(&scope, page.items()[0].reference()))?
            .ok_or("synthetic Bridge body missing")?;
        assert_eq!(body.text_body(), Some(BRIDGE_BODY));
