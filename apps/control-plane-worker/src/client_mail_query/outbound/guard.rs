...
pub(super) async fn source_is_accessible(
    env: &Env,
    actor: &ActorContext,
    client_id: &ClientId,
    ...
    {
        Ok(Some(_)) => Ok(true),
        Ok(None) | Err(QueryApplicationError::InvalidInput) => Ok(false),
        Err(QueryApplicationError::IntegrityFailure | QueryApplicationError::DependencyUnavailable) => {
            Err(worker::Error::RustError("client mail source lookup failed".to_owned()))
        }
    }
}
