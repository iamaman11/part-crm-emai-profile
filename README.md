...
3. Один профиль имеет только одного writer через Durable Object lease, monotonic
   epoch/fencing token и локальный OS lock.
4. Firefox lock-файлы никогда не удаляются автоматически.
5. Snapshot создается только после graceful close и подтвержденного quiescence.
6. R2 не используется как live filesystem: generation сначала материализуется
   на локальный диск.
7. Cookies, localStorage, IndexedDB, fingerprint data и mailbox secrets являются
   credential-equivalent данными.
8. Пароли, proxy credentials и OAuth tokens хранятся только как secret handles.
9. Качество профиля утверждается только versioned certification report;
   абсолютная невидимость не обещается.
10. Assignment профиля клиенту не является правом доступа.
11. Member без явного grant не видит и не запускает профиль.
12. Pure domains не зависят от Cloudflare, Windows, Python, browser или storage SDK.
13. D1, Durable Objects и R2 связываются idempotency, immutable objects, outbox и
    reconciliation, а не фиктивной общей транзакцией.
14. Статус ADR и readiness берётся из `ADR_STATUS.md` и `status.json`.
