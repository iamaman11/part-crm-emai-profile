-- PAS-2 Transaction D: retire the transitional request_digest column after the
-- successor runtime is proven active and all legacy readers/writers are retired.
--
-- Trusted payload_fingerprint rows are retained unchanged. Legacy rows that still
-- carry only request_digest become payload_fingerprint=NULL fail-closed tombstones.
-- Historical digest bytes are never copied or reclassified as trusted fingerprints.

ALTER TABLE idempotency_records
    RENAME TO idempotency_records_pas2_contract_legacy;
...
SELECT
    ...
    created_at_ms,
    expires_at_ms
FROM idempotency_records_pas2_contract_legacy;

DROP TABLE idempotency_records_pas2_contract_legacy;
...
-- Rebuild the outbound-mail FK family so the retired request_digest column is
-- physically removed without weakening the existing access/state constraints.
DROP TRIGGER outbound_mail_request_digest_immutable;
DROP TRIGGER outbound_mail_payload_fingerprint_immutable;
...
ALTER TABLE outbound_mail_ambiguity_marks
    RENAME TO outbound_mail_ambiguity_marks_pas2_contract_legacy;
ALTER TABLE outbound_mail_dispatch_completions
    RENAME TO outbound_mail_dispatch_completions_pas2_contract_legacy;
ALTER TABLE outbound_mail_dispatch_claims
    RENAME TO outbound_mail_dispatch_claims_pas2_contract_legacy;
ALTER TABLE outbound_mail_intents
    RENAME TO outbound_mail_intents_pas2_contract_legacy;
...
SELECT
    ...
    created_at_ms,
    updated_at_ms
FROM outbound_mail_intents_pas2_contract_legacy;
...
FROM outbound_mail_dispatch_claims_pas2_contract_legacy;
...
FROM outbound_mail_dispatch_completions_pas2_contract_legacy;
...
FROM outbound_mail_ambiguity_marks_pas2_contract_legacy;

DROP TABLE outbound_mail_ambiguity_marks_pas2_contract_legacy;
DROP TABLE outbound_mail_dispatch_completions_pas2_contract_legacy;
DROP TABLE outbound_mail_dispatch_claims_pas2_contract_legacy;
DROP TABLE outbound_mail_intents_pas2_contract_legacy;
