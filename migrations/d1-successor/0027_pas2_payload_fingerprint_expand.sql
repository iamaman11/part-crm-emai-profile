-- PAS-2 Transaction A: data-preserving bridge between the retired request_digest
-- representation and the server-owned payload_fingerprint representation.
--
-- The bridge is intentionally temporary. Every row has exactly one trust
-- representation: legacy rows keep request_digest and have payload_fingerprint=NULL;
-- successor-runtime rows have request_digest=NULL and a validated server-owned
-- payload_fingerprint. Historical digest bytes are never copied or reclassified as
-- trusted fingerprints.

ALTER TABLE idempotency_records
    RENAME TO idempotency_records_pas2_expand_legacy;
...
CREATE TABLE idempotency_records (
    ...
    request_digest TEXT
        CHECK(request_digest IS NULL OR length(request_digest) BETWEEN 16 AND 256),
    ...
    CHECK(
        (request_digest IS NOT NULL AND payload_fingerprint IS NULL)
        OR (request_digest IS NULL AND payload_fingerprint IS NOT NULL)
    )
) STRICT;
...
SELECT
    ...
    created_at_ms,
    expires_at_ms
FROM idempotency_records_pas2_expand_legacy;

DROP TABLE idempotency_records_pas2_expand_legacy;

CREATE TRIGGER idempotency_request_digest_immutable
BEFORE UPDATE OF request_digest ON idempotency_records
FOR EACH ROW
WHEN NEW.request_digest IS NOT OLD.request_digest
BEGIN
    SELECT RAISE(ABORT, 'idempotency_request_digest_immutable');
END;

CREATE TRIGGER idempotency_payload_fingerprint_immutable
BEFORE UPDATE OF payload_fingerprint ON idempotency_records
FOR EACH ROW
WHEN NEW.payload_fingerprint IS NOT OLD.payload_fingerprint
BEGIN
    SELECT RAISE(ABORT, 'idempotency_payload_fingerprint_immutable');
END;

-- Rebuild the outbound-mail FK family so child foreign keys continue to point to
-- the canonical table after the parent gains the temporary bridge columns.
...
ALTER TABLE outbound_mail_ambiguity_marks
    RENAME TO outbound_mail_ambiguity_marks_pas2_expand_legacy;
ALTER TABLE outbound_mail_dispatch_completions
    RENAME TO outbound_mail_dispatch_completions_pas2_expand_legacy;
ALTER TABLE outbound_mail_dispatch_claims
    RENAME TO outbound_mail_dispatch_claims_pas2_expand_legacy;
ALTER TABLE outbound_mail_intents
    RENAME TO outbound_mail_intents_pas2_expand_legacy;
...
SELECT
    ...
    created_at_ms,
    updated_at_ms
FROM outbound_mail_intents_pas2_expand_legacy;
...
FROM outbound_mail_dispatch_claims_pas2_expand_legacy;
...
FROM outbound_mail_dispatch_completions_pas2_expand_legacy;
...
FROM outbound_mail_ambiguity_marks_pas2_expand_legacy;

DROP TABLE outbound_mail_ambiguity_marks_pas2_expand_legacy;
DROP TABLE outbound_mail_dispatch_completions_pas2_expand_legacy;
DROP TABLE outbound_mail_dispatch_claims_pas2_expand_legacy;
DROP TABLE outbound_mail_intents_pas2_expand_legacy;

CREATE TRIGGER outbound_mail_request_digest_immutable
BEFORE UPDATE OF request_digest ON outbound_mail_intents
FOR EACH ROW
WHEN NEW.request_digest IS NOT OLD.request_digest
BEGIN
    SELECT RAISE(ABORT, 'outbound_mail_request_digest_immutable');
END;
