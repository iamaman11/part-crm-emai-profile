...
def validate(root: Path) -> list[str]:
    ...
    if status.get("schema_version") != 4 or status.get("as_of") != STATUS_DATE:
        errors.append("docs/status.json must be the current AR-3 schema/date projection")
    if status.get("production_ready") is not False:
        errors.append("production_ready must remain false throughout accepted AR-3")
    ...
    if current.get("architecture_complete") is not False or current.get("production_core_gate") != "BLOCKED":
        errors.append("AR-3 architecture_complete/Production Core gate state must remain fail closed")
    ...
    if transition.get("schema_version") != 7 or transition.get("status") != "ACTIVE_AFTER_ACCEPTED_AR3_MERGE":
    ...
    if transition.get("current_slice") != CURRENT_SLICE or transition.get("next_slice_after_acceptance") != NEXT_SLICE:
        errors.append("architecture transition must encode AR-3 -> AR-4A sequencing")
    ...
    if transition_state.get("architecture_complete") is not False or transition_state.get("production_core_gate") != "BLOCKED" or transition_state.get("production_ready") is not False:
        errors.append("transition state must remain fail closed through AR-3")
    ...
    if program_state.get("accepted_architecture_slices") != ACCEPTED_SLICES or program_state.get("current_architecture_slice") != CURRENT_SLICE or program_state.get("next_architecture_slice_after_acceptance") != NEXT_SLICE:
        errors.append("architecture inventory AR-3 program state is stale")
    ...
    common = ("Architecture Re-baseline v3", "issue #266", "AR-3", "AR-4A", "production_ready=false")
    require(root_readme, common, "README.md", errors)
    require(docs_readme, common, "docs/README.md", errors)
    require(index, ("CURRENT_AUTHORITY", "ARCHITECTURE_REBASELINE_V3_PLAN.md", "issue #266", "AR-3", "AR-4A"), "docs/INDEX.md", errors)
...
def self_test(root: Path) -> bool:
    ...
    fixtures = [
        ...
        ("slice rollback", Path("docs/status.json"), '"current_slice": "AR-3"', '"current_slice": "AR-2"', "current_slice"),
    ...
    for label, relative, old, new, expected in fixtures:
        with tempfile.TemporaryDirectory(prefix="ar3-document-authority-") as directory:
            ...
            if not errors or not any(expected.lower() in error.lower() for error in errors):
                ...
                return False
    print("Architecture Re-baseline v3 AR-3 documentation authority negative fixtures passed.")
    return True
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    print("Architecture Re-baseline v3 AR-3 documentation/program authority is consistent.")
    return 0
