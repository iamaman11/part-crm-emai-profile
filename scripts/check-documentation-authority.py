...
ACCEPTED_SLICES = ["AR-0", "AR-1", "AR-2", "AR-3", "AR-4A"]
...
def validate(root: Path) -> list[str]:
    ...
    if status.get("schema_version") != 4 or status.get("as_of") != STATUS_DATE:
        errors.append("docs/status.json must be the current AR-4A schema/date projection")
    if status.get("production_ready") is not False:
        errors.append("production_ready must remain false throughout accepted AR-4A")
    ...
    if current.get("architecture_complete") is not False or current.get("production_core_gate") != "BLOCKED":
        errors.append("AR-4A architecture_complete/Production Core gate state must remain fail closed")
    ...
    if transition.get("schema_version") != 7 or transition.get("status") != "ACTIVE_AFTER_ACCEPTED_AR4A_MERGE":
    ...
    if transition.get("current_slice") != CURRENT_SLICE or transition.get("next_slice_after_acceptance") != NEXT_SLICE:
        errors.append("architecture transition must encode AR-4A -> AR-4B sequencing")
    ...
    if transition_state.get("architecture_complete") is not False or transition_state.get("production_core_gate") != "BLOCKED" or transition_state.get("production_ready") is not False:
        errors.append("transition state must remain fail closed through AR-4A")
    ...
    if program_state.get("accepted_architecture_slices") != ACCEPTED_SLICES or program_state.get("current_architecture_slice") != CURRENT_SLICE or program_state.get("next_architecture_slice_after_acceptance") != NEXT_SLICE:
        errors.append("architecture inventory AR-4A program state is stale")
    ...
    common = ("Architecture Re-baseline v3", "issue #266", "AR-4A", "AR-4B", "production_ready=false")
    require(root_readme, common, "README.md", errors)
    require(docs_readme, common, "docs/README.md", errors)
    require(index, ("CURRENT_AUTHORITY", "ARCHITECTURE_REBASELINE_V3_PLAN.md", "issue #266", "AR-4A", "AR-4B"), "docs/INDEX.md", errors)
...
def self_test(root: Path) -> bool:
    ...
    fixtures = [
        ...
        ("slice rollback", Path("docs/status.json"), '"current_slice": "AR-4A"', '"current_slice": "AR-3"', "current_slice"),
    ...
    for label, relative, old, new, expected in fixtures:
        with tempfile.TemporaryDirectory(prefix="ar3-document-authority-") as directory:
            ...
            if not errors or not any(expected.lower() in error.lower() for error in errors):
                ...
                return False
    print("Architecture Re-baseline v3 AR-4A documentation authority negative fixtures passed.")
    return True
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    print("Architecture Re-baseline v3 AR-4A documentation/program authority is consistent.")
    return 0
