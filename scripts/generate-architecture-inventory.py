...
DOCUMENT_STATUS = [
    ...
    {"path": AR4B_EVIDENCE, "status": "EVIDENCE", "scope": "ar4b_client_mail_route_ownership_candidate"},
...
def validate_docs() -> None:
    ...
    if status.get("production_ready") is not False:
        raise SystemExit("docs/status.json must remain production_ready=false during accepted AR-4A / candidate AR-4B")
    if current.get("architecture_complete") is not False or current.get("production_core_gate") != "BLOCKED":
        raise SystemExit("docs/status.json must keep AR-4A architecture/gate state fail closed during candidate AR-4B")
    ...
    if program.get("accepted_slices") != ACCEPTED_SLICES or program.get("current_slice") != CURRENT_SLICE or program.get("next_slice_after_acceptance") != NEXT_SLICE:
        raise SystemExit("docs/status.json must project accepted AR-4A -> active AR-4B sequencing until AR-4B closeout")
...
def build_inventory() -> dict[str, object]:
    ...
    return {
        ...
        "documentation_authority": {
            ...
            "application_architecture_candidate_evidence": AR4B_EVIDENCE,
...
def self_test(expected: dict[str, object]) -> None:
    ...
    print("Architecture inventory AR-4B candidate negative self-test passed.")
...
def main() -> int:
    ...
    if args.write:
        ...
        print("Architecture inventory and AR-4B candidate ownership projection are current.")
    else:
        self_test(expected)
    return 0
