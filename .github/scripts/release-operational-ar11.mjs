...
function requireMarkers(text, markers, label) {
  ...
  for (const marker of markers) {
    if (!text.includes(marker)) errors.push(`${label} is missing ${JSON.stringify(marker)}`);
  }
  return errors;
}
...
function forbidMarkers(text, markers, label) {
  ...
  for (const marker of markers) {
    if (lower.includes(marker.toLowerCase())) {
      errors.push(`${label} contains forbidden authority ${JSON.stringify(marker)}`);
    }
  }
  return errors;
}
...
function operationalErrors({ requireCutover = true } = {}) {
  ...
  if ((promotion.match(/deploy \^?\\?$/gm) ?? []).length > 2) {
    errors.push('Release Set promotion has an unexpected deploy surface count');
  }

  const registry = JSON.parse(read(REGISTRY));
  const workflows = registry.active_registrations ?? registry.workflows ?? [];
  const buildRows = workflows.filter((row) => row.path === BUILD);
  const promotionRows = workflows.filter((row) => row.path === PROMOTION);
  if (buildRows.length !== 1) errors.push('Release Set build must appear exactly once in canonical Actions registry');
  if (promotionRows.length !== 1 || promotionRows[0].kind !== 'CURRENT_MANUAL_OPERATION') {
    errors.push('Release Set promotion must be the single canonical CURRENT_MANUAL_OPERATION');
  }
  const manualRows = workflows.filter((row) => row.kind === 'CURRENT_MANUAL_OPERATION');
  ...
  for (const key of ['build_once', 'promotion_rebuild', 'opsctl_network', 'opsctl_credentials', 'opsctl_provider_mutation', 'production_execution_before_ar17']) {
    if (!(key in policy)) errors.push(`AR-11 promotion policy is missing ${key}`);
  }
...
function selfTest() {
  const promotion = read(PROMOTION);
  const negative = forbidMarkers(
    `${promotion}\n  environment: production\n  run: cargo build --release\n`,
    ['environment: production', 'cargo build'],
    'negative fixture',
  );
