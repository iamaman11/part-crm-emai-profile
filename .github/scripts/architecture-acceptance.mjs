...
function fail(message) { throw new Error(`architecture acceptance: ${message}`); }
...
function sha(value, label) { if (!/^[0-9a-f]{40}$/.test(String(value ?? ''))) fail(`${label} must be an exact 40-hex SHA`); }
function load() { return { policy: readJson(POLICY_PATH), sequence: readJson(SEQUENCE_PATH) }; }
...
function expectedAcceptanceState(policy, slice) {
  const states = policy.acceptance_state ?? {};
  const wanted = slice === 'AR-17' ? states.ar17 : states.before_ar17;
  if (!wanted || typeof wanted !== 'object') fail(`missing acceptance state policy for ${slice}`);
  return wanted;
}
...
function validatePolicy(policy, slices) {
  ...
  const requiredStatus = policy.required_status_contexts ?? {};
  if (!Array.isArray(requiredStatus.sources) || requiredStatus.sources.length === 0 || requiredStatus.duplicate_contexts_forbidden !== true || requiredStatus.all_must_succeed_on_exact_candidate !== true) fail('required status context authority drifted');
  for (const source of requiredStatus.sources) {
    if (typeof source?.path !== 'string' || typeof source?.json_pointer !== 'string' || !source.json_pointer.startsWith('/')) fail('required status context source is malformed');
  }
  ...
  const before = expectedAcceptanceState(policy, 'AR-16');
  const ar17 = expectedAcceptanceState(policy, 'AR-17');
  if (before.architecture_complete !== false || before.production_core_gate !== 'BLOCKED' || before.production_ready !== false || before.production_mutation !== false) fail('pre-AR-17 acceptance state drifted');
  if (ar17.architecture_complete !== true || ar17.production_core_gate !== 'AUTHORIZED' || ar17.production_ready !== false || ar17.production_mutation !== false || policy.acceptance_state.first_production_enablement_stage !== 'PC-1') fail('AR-17/PC-1 state boundary drifted');
...
function validateBootstrap(policy) {
  ...
  const expected = expectedAcceptanceState(policy, 'AR-11');
  if (record.issue !== 372 || record.pr !== 374 || record.required_status_contexts_total !== 23 || record.required_status_contexts_success !== 23 || record.applicable_permanent_workflows_total !== 17 || record.applicable_permanent_workflows_success !== 17 || record.behind_by !== 0 || record.blocking_reviews !== 0 || record.unresolved_review_threads !== 0 || record.architecture_complete !== expected.architecture_complete || record.production_core_gate !== expected.production_core_gate || record.production_ready !== expected.production_ready || record.production_mutation !== expected.production_mutation) fail('AR-11 bootstrap acceptance evidence drifted');
  ...
  if (gitText(['rev-parse', `${record.merge_sha}^{tree}`]) !== record.merge_tree || gitText(['rev-parse', `${record.merge_sha}^1`]) !== record.base_sha) fail('accepted AR-11 merge identity drifted');
...
function scanForbidden() {
  ...
  for (const relative of roots) {
    ...
    for (const name of readdirSync(dir)) if (namePattern.test(name)) fail(`retired per-slice closeout executable remains in current tree: ${relative}/${name}`);
  }
}
...
function validateRecordObject(record, policy, slices, { verifyCurrent = false } = {}) {
  if (!record || typeof record !== 'object' || Array.isArray(record) || record.schema_version !== policy.acceptance_metadata.record_schema_version) fail('acceptance record schema drifted');
  for (const field of policy.acceptance_metadata.required_fields) if (!Object.hasOwn(record, field)) fail(`acceptance record missing ${field}`);
  const slice = normalizeSlice(record.slice);
  if (!slice || slice !== record.slice || !slices.some((item) => item.id === slice)) fail(`acceptance record contains unknown slice ${record.slice}`);
  for (const key of ['base_sha', 'candidate_sha', 'candidate_tree', 'merge_sha', 'merge_tree', 'accepted_main_reread']) sha(record[key], `acceptance ${key}`);
  for (const key of ['pr', 'required_status_contexts_total', 'required_status_contexts_success', 'applicable_permanent_workflows_total', 'applicable_permanent_workflows_success', 'behind_by', 'blocking_reviews', 'unresolved_review_threads']) {
    if (!Number.isInteger(record[key]) || record[key] < 0) fail(`acceptance ${key} must be a non-negative integer`);
  }
  if (record.pr < 1 || record.required_status_contexts_total < 1 || record.applicable_permanent_workflows_total < 1) fail('acceptance record must bind a PR and non-empty hosted verification sets');
  if (record.required_status_contexts_total !== record.required_status_contexts_success || record.applicable_permanent_workflows_total !== record.applicable_permanent_workflows_success) fail('acceptance record contains incomplete hosted verification');
  if (record.behind_by !== 0 || record.blocking_reviews !== 0 || record.unresolved_review_threads !== 0) fail('acceptance record contains merge blockers');
  const expected = expectedAcceptanceState(policy, slice);
  for (const key of ['architecture_complete', 'production_core_gate', 'production_ready', 'production_mutation']) if (record[key] !== expected[key]) fail(`acceptance record ${slice} has invalid ${key}`);
  if (record.candidate_tree !== record.merge_tree || record.accepted_main_reread !== record.merge_sha) fail('acceptance tree/reread identity mismatch');
  if (gitText(['rev-parse', `${record.merge_sha}^{tree}`]) !== record.merge_tree || gitText(['rev-parse', `${record.merge_sha}^1`]) !== record.base_sha) fail('acceptance merge identity does not match Git history');
  const candidate = git(['cat-file', '-e', `${record.candidate_sha}^{commit}`], { check: false });
  if (candidate.status === 0 && gitText(['rev-parse', `${record.candidate_sha}^{tree}`]) !== record.candidate_tree) fail('acceptance candidate tree does not match candidate commit');
  if (verifyCurrent) {
    const state = derive(policy, slices);
    if (state.current_slice !== slice) fail(`acceptance record is not for derived current slice: expected ${state.current_slice}, got ${slice}`);
  }
  return slice;
}

function parseTagMessage(tag, policy, slices) {
  if (gitText(['cat-file', '-t', tag]) !== 'tag') fail(`architecture acceptance ref must be an annotated tag: ${tag}`);
  ...
  validateRecordObject(record, policy, slices);
  return record;
}
...
function acceptedTagRecords(policy, slices) {
  ...
  for (const tag of result.stdout.split(/\r?\n/).filter(Boolean)) {
    ...
    const record = parseTagMessage(tag, policy, slices);
    if (record.slice !== expectedSlice) fail(`acceptance tag/path slice mismatch: ${tag}`);
    if (record.merge_sha !== gitText(['rev-list', '-n', '1', tag])) fail(`acceptance tag ${tag} does not target its merge_sha`);
...
function derive(policy, slices) {
  ...
  const state = expectedAcceptanceState(policy, accepted.id);
  return { schema_version: 1, accepted_checkpoint: accepted.id, current_slice: accepted.successor, ...state };
}
...
function premerge(args, policy, slices) {
  ...
  if (gitText(['merge-base', args.base, args.head]) !== args.base) fail('behind_by must be zero: candidate does not contain exact premerge main base');
  const state = derive(policy, slices);
  if (state.current_slice !== slice) fail(`slice ${slice} is not the derived current slice; expected ${state.current_slice}`);
  return { architecture_slice: true, slice, base_sha: args.base, candidate_sha: args.head, candidate_tree: gitText(['rev-parse', `${args.head}^{tree}`]), derived_state: state };
}

function record(args, policy, slices) {
  if (!args.file) fail('record requires --file');
  const absolute = path.resolve(ROOT, args.file);
  const relative = path.relative(ROOT, absolute);
  if (relative.startsWith('..') || path.isAbsolute(relative)) fail('record file must stay under repository workspace');
  const payload = readJson(relative);
  const slice = validateRecordObject(payload, policy, slices, { verifyCurrent: true });
  return { schema_version: 1, slice, tag: `${policy.acceptance_metadata.namespace}${slice.toLowerCase()}`, merge_sha: payload.merge_sha, record: payload };
}
...
function selfTest(policy, sequence) {
  ...
  rejected = false;
  try { validatePolicy(badPolicy, slices); } catch { rejected = true; }
  ...
  const badRecord = { ...structuredClone(policy.state_derivation.migration_bootstrap_acceptance), schema_version: 1, slice: 'AR-12' };
  badRecord.required_status_contexts_success = badRecord.required_status_contexts_total - 1;
  rejected = false;
  try { validateRecordObject(badRecord, policy, slices); } catch { rejected = true; }
  if (!rejected) fail('incomplete hosted verification acceptance record negative fixture passed');
  const ar17 = expectedAcceptanceState(policy, 'AR-17');
  if (ar17.architecture_complete !== true || ar17.production_core_gate !== 'AUTHORIZED' || ar17.production_ready !== false) fail('AR-17 state fixture is invalid');
}
...
if (args.command === 'contract') console.log('One-merge architecture acceptance contract passed.');
else if (args.command === 'derive') console.log(JSON.stringify(derive(policy, slices)));
else if (args.command === 'premerge') console.log(JSON.stringify(premerge(args, policy, slices)));
else if (args.command === 'record') console.log(JSON.stringify(record(args, policy, slices)));
else if (args.command === 'self-test') { selfTest(policy, sequence); console.log('Architecture acceptance negative matrix passed.'); }
else fail(`unknown command ${args.command}; expected contract, derive, premerge, record, self-test`);
