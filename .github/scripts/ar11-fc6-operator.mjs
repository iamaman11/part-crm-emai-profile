...
function parseWorkflowRun(event) {
  ...
  if (run.head_repository?.full_name !== REPOSITORY || !SHA.test(run.head_sha ?? '')) {
    fail('operator trigger gate must use an exact same-repository head SHA');
  }
...
function selectFallbackPullNumber(pulls, workflowRun) {
  ...
  const exact = pulls.filter((pull) =>
    pull?.state === 'open'
    && pull?.base?.ref === 'main'
    && pull?.head?.ref === workflowRun.headBranch
    && pull?.head?.sha === workflowRun.headSha
...
function auditMarker(request) {
  return `<!-- AR11_FC6_OPERATOR_AUDIT request=${request.requestId} -->`;
}

function auditBody(request, status, extra = []) {
  return [
    auditMarker(request),
    `FC-6 operator transport status: **${status}**.`,
    `Request: \`${request.requestId}\``,
    `Operation: \`${request.operation}\``,
    `Target Release Set: \`${request.releaseSetId}\``,
    `Expected current: \`${request.expectedCurrent}\``,
    ...extra,
    'The transport has no provider credentials and performs no provider mutation. Provider mutation remains exclusively inside the canonical Release Set Promotion workflow.',
  ].join('\n\n');
}
...
async function ensureAuditComment(issueNumber, request) {
  ...
  const created = await github(`/repos/${REPOSITORY}/issues/${issueNumber}/comments`, {
    method: 'POST',
    body: { body: auditBody(request, 'VALIDATED / DISPATCH PENDING') },
  });
  if (!Number.isInteger(created?.id) || created.id <= 0) fail('created operator audit comment id is invalid');
  return created.id;
}

async function updateAuditComment(commentId, body) {
  await github(`/repos/${REPOSITORY}/issues/comments/${commentId}`, { method: 'PATCH', body: { body } });
}
...
function selfTest() {
  ...
  const missingAssociation = structuredClone(workflowRun);
  missingAssociation.workflow_run.pull_requests = [];
  ...
  const ambiguousAssociation = structuredClone(workflowRun);
  ambiguousAssociation.workflow_run.pull_requests = [{ number: 77 }, { number: 78 }];
  ...
  if (parseRequest({ repository: { full_name: REPOSITORY }, issue: { number: ISSUE_NUMBER } }) !== null) {
    fail('issue comments must not be an operator authority');
  }
  ...
  const fallbackRows = [{
    ...
    state: 'open',
    base: { ref: 'main' },
    head: { ref: fallbackRun.headBranch, sha: fallbackRun.headSha, repo: { full_name: REPOSITORY } },
  }];
  ...
  const boundRuns = [
    { id: 101, event: 'workflow_dispatch', head_branch: 'main', path: '.github/workflows/release-set-promotion.yml', display_title: 'AR11 promote pr-77' },
    { id: 102, event: 'pull_request', head_branch: 'main', path: '.github/workflows/release-set-promotion.yml', display_title: 'AR11 promote pr-77' },
  ];
  ...
  if (resolveWorkflowRunRequest.toString().includes('pull.body')) fail('mutable PR body must never be operator authority');
...
async function main() {
  ...
  try {
    ...
    await updateAuditComment(auditCommentId, auditBody(request, 'ACCEPTED', [
      `Canonical run: ${run.html_url}`,
      `Run id: ${run.id}`,
      `Reused existing canonical run: \`${reused}\``,
    ]));
    ...
    if (auditCommentId && request && responseIssue) {
      ...
      try {
        await updateAuditComment(auditCommentId, auditBody(request, 'FAILED / NO NEW AUTHORITY', [`Failure: \`${message.slice(0, 600).replace(/`/g, "'")}\``]));
      } catch (commentError) {
        console.error(`failed to update operator audit comment after rejection: ${commentError instanceof Error ? commentError.message : commentError}`);
      }
    }
    throw error;
  }
